...
async def get_user(user_id: int, db: _orm.Session = _fastapi.Depends(_utils.get_db)):
...
async def verify_user(user_id: int, db: _orm.Session = _fastapi.Depends(_utils.get_db)):
    user = await _services.get_user_by_id(user_id=user_id, db=db)
    if user is None:
        raise _fastapi.HTTPException(status_code=404, detail="User tidak ada!")
